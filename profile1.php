...
<?php
  ...
  if($news->num_rows>0)
  {
    ...
    while($ne=$news->fetch_assoc())
    {
     ...
     if (! is_null($c_id))
     {
         if($conn->query("select * from comment where comment_id='$c_id'")->num_rows>0){
             $comment=$conn->query("select * from comment where comment_id='$c_id'")->fetch_assoc();
        ...
        echo " commented ".$comment["comment"]." on ".$answer["answer_name"]." on the question ".$question["question_name"];
         }
     } 
